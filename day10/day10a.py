...
class Bot(object):
	def __init__(self, number, chips, low_type, low, high_type, high):
	...
	def compare(self):
		if len(self.chips) == 2:
			#print "Bot %d: I will compare %d and %d" % (self.number, self.chips[0], self.chips[1])
			self.chips.sort()
			if self.chips[0] == 17 and self.chips[1] == 61:
				print "Bot %d is responsible for sorting 17 and 61." % self.number
			if self.low_type == 'bot':
				bots[self.low].set_chip(self.chips[0])
			#else:
				#print "Bot %d: Discarding chip %d to low output %d" % (self.number, self.chips[0], self.low)
			if self.high_type == 'bot':
				bots[self.high].set_chip(self.chips[1])
			#else:
				#print "Bot %d: Discarding chip %d to high output %d" % (self.number, self.chips[1], self.high)
...
for line in input.split('\n'):
	m = re.search('bot\s(?P<bot_number>[0-9]+)\sgives\slow\sto\s(?P<bot_low_type>(bot|output))\s(?P<bot_low>[0-9]+)\sand\shigh\sto\s(?P<bot_high_type>(bot|output))\s(?P<bot_high>[0-9]+)', line)
	if m:
		bots[int(m.group('bot_number'))] = Bot(m.group('bot_number'), None, m.group('bot_low_type'), m.group('bot_low'), m.group('bot_high_type'), m.group('bot_high'))

for line in input.split('\n'):
	m = re.search('value\s(?P<chip>[0-9]+)\sgoes\sto\sbot\s(?P<bot>[0-9]+)', line)
	if m:
		bots[int(m.group('bot'))].set_chip(int(m.group('chip')))

while True:
	for bot_number, bot in bots.iteritems():
		bot.compare()
