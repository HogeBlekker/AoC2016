...
def get_valid_sector_id(encrypted_name, checksum, checksum_length = 5):
	encrypted_name = encrypted_name.replace('-', '')
	dictionary = {}
	for m in re.finditer(r"[a-z]{1}", encrypted_name):
		if m.group(0) in dictionary:
			dictionary[m.group(0)] += 1
		else:
			dictionary[m.group(0)] = 1

	listed_dictionary = []
	for w in sorted(dictionary, key=dictionary.get, reverse=True):
		listed_dictionary.append([w, dictionary[w]])

	# Using Python's Sort Stability
	listed_dictionary = sorted(listed_dictionary, key = itemgetter(0), reverse=False)
	listed_dictionary = sorted(listed_dictionary, key = itemgetter(1), reverse=True)

	result = ''
	for item in listed_dictionary[0:checksum_length]:
		result += item[0]

	if result == checksum:
		sector_id = re.search("[0-9]+", encrypted_name).group(0)
		return int(sector_id)
	return False


def rotate_cipher(encrypted_name, sector_id):
	return rot_alpha(sector_id)(encrypted_name)
...
for line in lines:
	m = re.search("(?P<encrypted_name>.*)\[(?P<checksum>[a-z]+)\]", line)
	sector_id += get_valid_sector_id(m.group('encrypted_name'), m.group('checksum'))
	sum_sector_ids += sector_id
	if sector_id:
		temp = re.search(r"([a-z-]*)([0-9]+$)", m.group('encrypted_name'))
		print "Room " + temp.group(2) + " contains " + rotate_cipher(temp.group(1).replace('-', ''), int(temp.group(2)))
		#print rotate_cipher('qzmt-zixmtkozy-ivhz', 343)
	sector_id = 0
